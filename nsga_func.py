...
def joist_carbon(b, h, cc, beamIndex):
    ...
    bI = [607.0, 548.0, 499.0, 433.0, 390.0, 343.0, 584.0, 494.0, 438.0, 415.0, 393.0, 350.0, 314.0, 272.0, 249.0, 222.0,
...
def joist_beam_vibration_level(b_floor, l_beam, l1, l2, b_st, h_st, cc, b_board, t_board, E0_mean_ST, E0_mean_fb, m,
    ...
    acceptance_class_R = np.where(R >= 32, 7, np.where(R >= 24, 6, np.where(R >= 16, 5, np.where(R >= 12, 4, np.where(R >= 8, 3,np.where(R >= 4, 2, 1))))))
    ...
    acceptance_class_W = np.where(W_1kN_sys>=1.6, 7, np.where(W_1kN_sys>=1.2, 6, np.where(W_1kN_sys>=0.8, 5, np.where(W_1kN_sys>=0.5, 4, np.where(W_1kN_sys>=0.25, 3, 2)))))
...
def joist_carbon_inc(b, h, cc, beamIndex):
    ...
    bI = [607.0, 548.0, 499.0, 433.0, 390.0, 343.0, 584.0, 494.0, 438.0, 415.0, 393.0, 350.0, 314.0, 272.0, 249.0,
              ...
              265.0,
              240.0, 217.0, 192.0, 170.0, 152.0, 140.0, 125.0, 551, 498, 455, 415, 372, 341, 307, 285, 262, 241, 217,
              195,
              174, 155, 238.0, 179.0, 149.0, 153.0, 140.0, 125.0, 113.0, 101.0, 92.0, 82.0, 138.0, 122.0, 109.0, 101.0,
              92.1,
              82.2, 85.0, 74.0, 66.0, 106.0, 98.3, 89.3, 82.0, 74.3, 67.1, 82.1, 74.2, 67.2, 59.8, 52.3, 85.0, 74.2,
              67.1,
              60.1, 54.1, 53.3, 46.0, 39.0, 67.1, 57.0, 51.0, 45.0, 39.1, 33.1, 54.0, 46.1, 40.3, 48.1, 41.9, 37.0,
              32.8,
              28.2, 24.8, 43.0, 37.0, 31.1, 28.3, 25.2, 22.0, 30.0, 25.1, 23.1, 19.0, 16.0, 13.0]
...
'''X = np.array([[0, 0, 400, 5], [5, 10, 600.81850517, 100]])
f1 = joist_beam_vibration_level_inc(6000, 6000, 3000, 3000, X[:, 0], X[:, 1], X[:, 2], 1000, 22, 11000, 9000, 55, X[:, 3], 1.8, 88, 1.28, 1, 1, 2, 1.5)
#def joist_beam_vibration_level(b_floor, l_beam, l1, l2, b_st, h_st, cc, b_board, t_board, E0_mean_ST, E0_mean_fb, m, beamIndex, a, b_BSEN, ke1j, ke1b, ke2b, sides_supp, fw):
print(f1[0])'''
